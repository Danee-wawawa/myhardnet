...
def get_fpn_rcnn_testbatch(roidb):
    ...
    area_threshold = {'P5': 160, 'P4': 80, 'P3': 40}
...
def get_fpn_maskrcnn_batch(roidb):
    ...
    if not config.ENABLE_OHEM:
      ...
      assert config.TRAIN.BATCH_ROIS_OHEM % config.TRAIN.BATCH_IMAGES == 0, \
          'BATCHIMAGES {} must divide BATCH_ROIS {}'.format(config.TRAIN.BATCH_IMAGES, config.TRAIN.BATCH_ROIS_OHEM)
      rois_per_image = config.TRAIN.BATCH_ROIS_OHEM / config.TRAIN.BATCH_IMAGES
    ...
    for im_i in range(num_images):
        ...
        if not config.USE_AFP:
          ...
          data_on_imgs.update({'img_%s' % im_i: {'rois_on_levels': im_rois_on_levels,
          ...
          data_on_imgs.update({'img_%s' % im_i: {'rois': im_rois,
                                                               'labels': labels,
...
def sample_rois_fpn(rois, assign_levels, fg_rois_per_image, rois_per_image, num_classes,
    ...
    :return: (rois, labels, bbox_targets, bbox_weights)
    """
    #assert (config.ENABLE_OHEM and config.USE_AFP) or (not config.ENABLE_OHEM and not config.USE_AFP)
    if config.ENABLE_OHEM:
      assert config.USE_AFP
    DEBUG = False
    if labels is None:
        if len(gt_boxes) == 0:
            gt_boxes = np.zeros((1, 5))
    ...
    #if config.ENABLE_OHEM:
    #  fg_rois_per_this_image = fg_indexes.size
    #else:
    #  fg_rois_per_this_image = np.minimum(fg_rois_per_image, fg_indexes.size)
    fg_rois_per_this_image = np.minimum(fg_rois_per_image, fg_indexes.size)
    ...
    # Compute number of background RoIs to take from this image (guarding against there being fewer than desired)
    #if config.ENABLE_OHEM:
    #  bg_rois_per_this_image = bg_indexes.size
    #else:
    #  bg_rois_per_this_image = rois_per_image - fg_rois_per_this_image
    #  bg_rois_per_this_image = np.minimum(bg_rois_per_this_image, bg_indexes.size)
    bg_rois_per_this_image = rois_per_image - fg_rois_per_this_image
    ...
    if mask_targets is not None:
        ...
        assert mask_inds is not None
        def _mask_umap(mask_targets, mask_labels, mask_inds):
            _mask_targets = np.zeros((num_rois, num_classes)+mask_size, dtype=np.int8)
