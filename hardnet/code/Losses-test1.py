...
def loss_HardNet(anchor, positive, anchor_swap = False, anchor_ave = False,\
    ...
    if batch_reduce == 'min':
        ...
        min_neg_3_test = torch.min(dist_without_min_on_diag,1)[0]
        min_neg_test = torch.min(dist_without_min_on_diag,1)[0]
        if anchor_swap:
        ...
        if False:
            ...
            min_neg_3_test = min_neg_3
            min_neg_test = min_neg
            min_neg = torch.min(min_neg,min_neg_3)
            print (min_neg_a)
            print (min_neg_p)
            print (min_neg_3)
            print (min_neg)
        min_neg_3_test = min_neg_3_test
    ...
    print('-------------------------------',min_neg_3_test,min_neg_test)
    print('-------------------------------',pos,min_neg)

    dist_matrix_test = distance_matrix_vector(min_neg_3_test, min_neg_test) +eps
    bet_neg = torch.diag(dist_matrix_test)

    if loss_type == "triplet_margin":
        loss = torch.clamp(margin + pos - min_neg, min=0.0)+torch.clamp(0.5 + pos - bet_neg, min=0.0)
    elif loss_type == 'softmax':
        exp_pos = torch.exp(2.0 - pos);
        exp_den = exp_pos + torch.exp(2.0 - min_neg) + eps;
        loss = - torch.log( exp_pos / exp_den )
