...
import zipfile
from text_detector_13 import TextDetector
script_path = './script_test_ch4/'
dataset_root = "/dataset/user/6af39c4a-01d9-11e9-b0aa-fa163ee59f29/966"
save_root = "/dataset/user/6af39c4a-01d9-11e9-b0aa-fa163ee59f29/1030/Text-Detection/Text-Detection"
lst_dir = 'imglists_13'
    ...
    with open(test_img_lst, 'r') as f:
        ...
        for line in lines:
            ...
            for mini_box in results:
                #print("---------------",mini_box)
                cv2.polylines(im, [mini_box],  1, (0,0,255),2)
            ...
            for line in lines:
                ...
                gt_box[1][0] = int(splits[0])
                gt_box[1][1] = int(splits[3])
                gt_box[2][0] = int(splits[2])
                gt_box[2][1] = int(splits[3])
                gt_box[3][0] = int(splits[2])
                gt_box[3][1] = int(splits[1])
                gt_box = np.int32(gt_box)
                cv2.polylines(im, [gt_box],  1, (0,255,0),2)
