...
seg_dir = 'seg_labels_17_crop'
lst_dir = 'imglists_17_crop'
crop_image_dir = 'icdar17_crop'

for _dir in [lst_dir, seg_dir, crop_image_dir]:
...
for image_set in ['train', 'test']:
  ...
  if image_set=='test':
    ...
    gt_dir = os.path.join(save_root, 'label','train')
    rotates=[0, 15, 30, 45]
    n_samples = 7200
  assert rotates[0]==0
  ...
  for imgid in range(1, n_samples+1):
    ...
    #print '--------------------w_image_path', w_image_path
        ...
        #rinst = inst
          for i in range(NUM_CROP):
            ...
            if Flag == True: 
              ...
              seg_file = os.path.join(dataset_root, seg_file_name)
              rinst.save(seg_file)
              _image_path = os.path.join(crop_image_dir, '%s_%d_%d_%.2f_%d.jpg'%(image_set, imgid, angle, crop_scale,i))
              _image_path = os.path.join(dataset_root, _image_path)
              rimg.save(_image_path)
              _w_image_path = '/'.join(_image_path.split('/')[-2:])
              #print '------------------_w_image_path', _w_image_path
    ...
    #  test_image_path = os.path.join(save_root, 'image', 'val', 'img_%d.jpg'%imgid)
    #  print '---------------------------test_path',test_image_path
    #  ##inst.save(seg_file)
    #  ##lst.write("%d\t%s\t%s\n"%(idx, w_image_path, seg_file_name))
    #  lst.write("%d\t%s\n"%(idx, test_image_path))
    #  idx+=1
