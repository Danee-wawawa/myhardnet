...
def residual_unit(data, num_filter, stride, dim_match, name):
    ...
    return sum
def ConvUnit(data, num_filter, kernel, pad, name, stride=(1, 1), act_type='relu'):
    data = mx.symbol.Convolution(data=data, num_filter=num_filter, kernel=kernel, stride=stride, pad=pad, name=name+'_unit')
    if len(act_type)>0:
        data  = mx.sym.Activation(data=data, act_type='relu', name=name + '_relu')
    return data
def ConvDeform(data, num_filter, kernel, pad, name, stride=(1, 1), act_type='relu', num_group=1):
    f = num_group*27
    deform_offset = mx.sym.Convolution(data=data, num_filter=f, kernel=kernel, stride=stride, pad=pad, name=name+'_offset')
    deform_offset_t = mx.sym.slice_axis(deform_offset, axis=1, begin=0, end=18)
    deform_mask =  mx.sym.slice_axis(deform_offset, axis=1, begin=18, end=None)
    deform_mask = 2 * mx.sym.Activation(deform_mask, act_type='sigmoid')
    data = mx.contrib.symbol.ModulatedDeformableConvolution(name=name + '_conv', data=data, offset=deform_offset_t, mask=deform_mask,
                       num_filter=num_filter, pad=pad, kernel=kernel, stride=stride, 
                       num_deformable_group=num_group, no_bias=True)
    if len(act_type)>0:
      data = mx.symbol.Activation(data=data, act_type=act_type, name=name+'_relu')
    return data

def inceptDCN_unit(data, num_filter, name):
    incept_conv0_0 = ConvUnit(data=data, num_filter=num_filter, kernel=(1,1), pad=(0,0), name=name+'_incept_conv0_0')
    incept_conv0_1 = ConvUnit(data=incept_conv0_0, num_filter=num_filter, kernel=(1,1), pad=(0,0), name=name+'_incept_conv0_1')
    incept_conv0_2 = ConvDeform(data=incept_conv0_1, num_filter=num_filter, kernel=(3,3), pad=(1,1), name=name+'_incept_conv0_2')

    incept_conv1_0 = ConvUnit(data=data, num_filter=num_filter, kernel=(1,1), pad=(0,0), name=name+'_incept_conv1_0')
    incept_conv1_1 = ConvUnit(data=incept_conv1_0, num_filter=num_filter, kernel=(1,3), pad=(0,1), name=name+'_incept_conv1_1')
    incept_conv1_2 = ConvUnit(data=incept_conv1_1, num_filter=num_filter, kernel=(3,1), pad=(1,0), name=name+'_incept_conv1_2')
    incept_conv1_3 = ConvDeform(data=incept_conv1_2, num_filter=num_filter, kernel=(3,3), pad=(1,1), name=name+'_incept_conv1_3')

    incept_conv2_0 = ConvUnit(data=data, num_filter=num_filter, kernel=(1,1), pad=(0,0), name=name+'_incept_conv2_0')
    incept_conv2_1 = ConvUnit(data=incept_conv2_0, num_filter=num_filter, kernel=(1,5), pad=(0,2), name=name+'_incept_conv2_1')
    incept_conv2_2 = ConvUnit(data=incept_conv2_1, num_filter=num_filter, kernel=(5,1), pad=(2,0), name=name+'_incept_conv2_2')
    incept_conv2_3 = ConvDeform(data=incept_conv2_2, num_filter=num_filter, kernel=(3,3), pad=(1,1), name=name+'_incept_conv2_3')

    incept_mixed = mx.symbol.Concat(*[incept_conv0_2, incept_conv1_3, incept_conv2_3])
    incept_subout = ConvUnit(data=incept_mixed, num_filter=num_filter, kernel=(1,1), pad=(0,0), act_type='', name=name+'_incept_mixed')
    shortcut = data
    data = shortcut + incept_subout
...
def get_resnet_conv_down(conv_feat):
    ...
    if config.USE_INCEPT:
        for k in conv_fpn_feat:
            d = conv_fpn_feat[k]
            conv_fpn_feat[k] = inceptDCN_unit(d, 256, name="INCEPT_%s"%k)
...
def get_resnet_fpn_maskrcnn(num_classes=config.NUM_CLASSES):
    ...
    for stride in rcnn_feat_stride:
        if config.ROIALIGN:
            ...
            offset_t = mx.contrib.sym.DeformablePSROIPooling(name='roi_offset_t_stride%s'%stride, data=_feat, rois=rois['rois_stride%s' % stride], group_size=1, pooled_size=pooled_size,
                                sample_per_part=4, no_trans=True, part_size=part_size, output_dim=256, spatial_scale=1.0/stride)
            offset_weight = mx.symbol.Variable('roi_offset_stride%s_weight'%stride, init=mx.init.Constant(0.0), attr={'__lr_mult__': '0.01'})
            offset_bias = mx.symbol.Variable('roi_offset_stride%s_bias'%stride, init=mx.init.Constant(0.0), attr={'__lr_mult__': '0.01'})
            offset = mx.sym.FullyConnected(name='roi_offset_stride%s'%stride, data=offset_t, num_hidden=part_size*part_size * 2, weight=offset_weight, bias=offset_bias)
            offset_reshape = mx.sym.Reshape(data=offset, shape=(-1, 2, part_size, part_size))
            roi_pool = mx.contrib.sym.DeformablePSROIPooling(name='deformable_roi_pool_stride%s'%stride, data=_feat, rois=rois['rois_stride%s' % stride],
                                trans=offset_reshape, group_size=1, pooled_size=pooled_size, sample_per_part=4,
                                no_trans=False, part_size=part_size, output_dim=256, spatial_scale=1.0/stride, trans_std=0.1)
