...
def get_image(roidb, scale=False):
    ...
    for i in range(num_images):
        roi_rec = roidb[i]
        if 'image_data' in roi_rec:
          im = roi_rec['image_data'].copy()
        else:
          assert os.path.exists(roi_rec['image']), '%s does not exist'.format(roi_rec['image'])
          im = Image.open(roi_rec['image'])
        if 'rotate' in roi_rec:
          rotate = roi_rec['rotate']
          im_size = im.size
          if rotate!=0:
            im = im.rotate(rotate, expand=True).resize(im_size)
        im = np.array(im)[:,:,::-1] #TO BGR
        if roidb[i]['flipped']:
            im = im[:, ::-1, :]
