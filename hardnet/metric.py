...
def get_rcnn_fpn_names():
    ...
    if not config.USE_AFP:
      ...
      label.append('rcnn_label')
      label.append('rcnn_bbox_target')
      label.append('rcnn_bbox_weight')
    return pred, label
...
class RPNAccMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        pred_label = mx.ndarray.argmax_channel(pred).asnumpy().astype(np.int32)
...
class RCNNAccMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        if not config.USE_AFP:
          for s in config.RCNN_FEAT_STRIDE:
              label.append(labels[self.label.index('rcnn_label_stride%s' % s)].asnumpy().reshape((-1,)).astype(np.int32))
          label = np.concatenate(label, axis=0)
        else:
          label = labels[self.label.index('rcnn_label')].asnumpy().reshape((-1,)).astype(np.int32)

        last_dim = pred.shape[-1]
        pred_label = pred.asnumpy().reshape(-1, last_dim).argmax(axis=1).astype(np.int32)
...
class MaskLogLossMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        if not config.USE_AFP:
          for s in config.RCNN_FEAT_STRIDE:
              label.append(labels[self.label.index('rcnn_label_stride%s' % s)].asnumpy().reshape((-1,)).astype(np.int32))
              mask_target.append(labels[self.label.index('mask_target_stride%s' % s)].asnumpy().reshape((-1, config.NUM_CLASSES, config.ROI_SIZE[0]*2,config.ROI_SIZE[1]*2)))
          ...
          label = labels[self.label.index('rcnn_label')].asnumpy().reshape((-1,)).astype(np.int32)
          mask_target = labels[self.label.index('mask_target')].asnumpy().reshape((-1, config.NUM_CLASSES, config.ROI_SIZE[0]*2,config.ROI_SIZE[1]*2))
